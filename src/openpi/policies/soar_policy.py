...
def make_soar_example() -> dict:
    """Creates a random input example for the SOAR policy."""
    return {
        'observation/state': np.random.rand(13),
        'observation/head_image': np.random.randint(256, size=(480, 640, 3), dtype=np.uint8),
        'observation/hand_image': np.random.randint(256, size=(480, 640, 3), dtype=np.uint8),
        'prompt': 'do something',
    }
...
class SoarInputs(transforms.DataTransformFn):
    """
    ...
    def __call__(self, data: dict) -> dict:
        ...
        slices = [slice(None)] * data['observation/state'].ndim
        slices[-1] = slice(0, self.action_dim)
        state = data['observation/state'][tuple(slices)]
        ...
        wrist_image = _parse_image(data['observation/hand_image'])
        ...
        if 'actions' in data:
            ...
            slices = [slice(None)] * data['actions'].ndim
            slices[-1] = slice(0, self.action_dim)
            actions = data['actions'][tuple(slices)]
...
@dataclasses.dataclass(frozen=True)
class SoarOutputs(transforms.DataTransformFn):
    """
